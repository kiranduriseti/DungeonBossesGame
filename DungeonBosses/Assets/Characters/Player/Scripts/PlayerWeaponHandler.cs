...
public class PlayerWeaponHandler : MonoBehaviour
{
    ...
    public void FireCurrentWeapon()
    {
        ...
        if (playerAmmo != null && !playerAmmo.HasAmmo(gun))
        {
            playerAmmo.OnAmmoEmpty?.Invoke();
            return;
        }
    ...
    public void EquipPrimary()
    {
        ...
        if (playerAmmo != null)
            playerAmmo.InvokeAmmoChanged(primaryGun);
    }
    ...
    public void EquipSecondary()
    {
        ...
        if (playerAmmo != null)
            playerAmmo.InvokeAmmoChanged(secondaryGun);
    }

    public void SwitchWeapon(float slot)
    {
        if (slot == 1)
            EquipPrimary();
        else if (slot == 2)
    ...
    {
        if (playerAmmo != null)
            playerAmmo.AddAmmo(GetCurrentGun());
    }

    public bool NeedsAmmo()
    {
        return playerAmmo != null && playerAmmo.NeedsAmmo(GetCurrentGun());
    }
    ...
    public void SwapCurrentWeapon(GameObject touchedPickupObject, GameObject newHeldGunPrefab)
    {
        ...
        if (usingPrimary)
            SwapPrimaryWeapon(touchedPickupObject, newHeldGunPrefab);
        else
            SwapSecondaryWeapon(touchedPickupObject, newHeldGunPrefab);
    }

    private void SwapPrimaryWeapon(GameObject touchedPickupObject, GameObject newHeldGunPrefab)
    {
        if (touchedPickupObject != null)
            Destroy(touchedPickupObject);
    ...
    private void SwapSecondaryWeapon(GameObject touchedPickupObject, GameObject newHeldGunPrefab)
    {
        if (touchedPickupObject != null)
            Destroy(touchedPickupObject);
